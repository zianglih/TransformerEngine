...
def _act_func(activation: str, recipe: Optional[Recipe] = None):
    ...
    if activation not in funcs:
        raise NotImplementedError(
            "Activation type " + activation + " is not supported!"
        )
...
class _LayerNormMLP(torch.autograd.Function):
    ...
    def forward(
        ...
        keep_backward_unquantized = (
            fp8 and FP8GlobalStateManager.keep_backward_unquantized()
        )
        ...
        if debug:  # turn off userbuffers in debug mode
            ...
            ub_bulk_dgrad = False
        ub_overlap_ag = (
            ub_overlap_ag and is_grad_enabled and not return_layernorm_output_gathered
        )
        ...
        if fp8:
            ...
            if (
                sequence_parallel
                and fc1_input_quantizer.supports_only_rowwise_all_gather()
            ):
            ...
            if fp8:
                ...
                if recipe.float8_block_scaling():
                    ...
                    act_out = activation_func(
                        fc1_out, fc2_input_quantizer, **act_params
                    )
            else:
                if fp8_calibration:
                    act_out = activation_func(fc1_out, None, **act_params)
        ...
        act_out_hp = act_out
        if keep_backward_unquantized and is_grad_enabled:
        ...
        if ub_overlap_rs:
            ...
            reduce_scatter_out = torch.empty(
                dim_size, dtype=activation_dtype, device=device
            )
    ...
    def backward(
        ...
        with get_nvtx_range_context("_LayerNormMLP_backward"):
            ...
            if ctx.fc2_weight_requires_grad:
                ...
                if use_quantized_bwd:
                    if isinstance(act_out, QuantizedTensorStorage):
                        ...
                        ctx.fc2_input_quantizer.set_usage(
                            rowwise=False, columnwise=True
                        )
                ...
                fc2_wgrad_gemm_kwargs = {
                    ...
                    "layout": "NT",
                    "out": origin_fc2_weight.main_grad
                    if ctx.fuse_wgrad_accumulation
                    else None,
                    "bias": fc2_bias
                ...
                if (
                    ...
                    if fc2_bias_grad is None:
                        if (
                            ...
                            fc2_bias_grad_ = act_out.view(-1, act_out.shape[-1]).sum(
                                dim=0
                            )
            ...
            if (
                ctx.wgrad_store is not None
                and not ctx.wgrad_store.delay_wgrad_compute()
            ):
            ...
            if ctx.bias_gelu_fusion:
                ...
                assert not ctx.fp8
                fc1_bias_grad, dact = bgrad_dgelu_fused(
                    fc2_dgrad, fc1_out_without_bias, fc1_bias
                )
            ...
            fc1_dgrad_shape = [
                reduce(multiply_op, inputmat.shape[:-1]),
                inputmat.shape[-1],
            ]
            ...
            if ctx.ub_overlap_rs_dgrad:
                ...
                if ctx.sequence_parallel:
                    if (
                        ...
                        and ctx.return_layernorm_output_gathered
                    ):
            ...
            if ctx.fc1_weight_requires_grad:
                ...
                if use_quantized_bwd:
                    if isinstance(ln_out_total, QuantizedTensorStorage):
                        ...
                        ctx.fc1_input_quantizer.set_usage(
                            rowwise=False, columnwise=True
                        )
                ...
                if use_quantized_bwd:
                    if isinstance(dact, QuantizedTensorStorage):
                        ...
                        ctx.fc1_grad_output_quantizer.set_usage(
                            rowwise=False, columnwise=True
                        )
                        dact = ctx.fc1_grad_output_quantizer(dact)
                ...
                fc1_wgrad_gemm_kwargs = {
                    ...
                    "layout": "NT",
                    "out": origin_fc1_weight.main_grad
                    if ctx.fuse_wgrad_accumulation
                    else None,
...
class LayerNormMLP(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        if self.use_bias:
            ...
            self.register_parameter(
                "fc1_bias", fc1_bias, init_fn=init_method_constant(0.0)
            )
        else:
        ...
        if self.use_bias:
            ...
            self.register_parameter(
                "fc2_bias", fc2_bias, init_fn=init_method_constant(0.0)
            )
        else:
    ...
    def reset_parameters(self, defer_init=False):
        ...
        if not defer_init:
            ...
            if self.normalization != "RMSNorm":
                setattr(
                    self.layer_norm_bias, "sequence_parallel", self.sequence_parallel
                )
    ...
    def forward(
        ...
        if self.ub_overlap_rs:
            if get_ub(
                "fc2_fprop", FP8GlobalStateManager.is_fp8_enabled()
            ).is_fp8_ubuf():
                fp8_output = True
    ...
    def _get_quantizers(self, fp8_output, is_grad_enabled):
        ...
        if self.fp8 or self.fp8_calibration:
            fc1_input_quantizer = self.quantizers["scaling_fwd"][
                tex.FP8FwdTensors.GEMM1_INPUT
            ]
            fc1_input_quantizer.internal = True
            fc2_input_quantizer = self.quantizers["scaling_fwd"][
    ...
    def onnx_forward(
        ...
        assert not TEDebugState.debug_enabled, (
        ...
        clamped_swiglu_limit, clamped_swiglu_alpha = (
            act_params.get("limit", 7.0),
            act_params.get("alpha", 1.702),
        )
        ...
        activation_map = {
            ...
            "geglu": lambda x: torch.nn.functional.gelu(x.chunk(2, -1)[0])
            * x.chunk(2, -1)[1],
            "qgelu": lambda x: torch.nn.functional.gelu(x, approximate="tanh"),
            "qgeglu": lambda x: torch.nn.functional.gelu(
                x.chunk(2, -1)[0], approximate="tanh"
            )
            * x.chunk(2, -1)[1],
            "relu": torch.nn.functional.relu,
            "reglu": lambda x: torch.nn.functional.relu(x.chunk(2, -1)[0])
            * x.chunk(2, -1)[1],
            "srelu": lambda x: torch.nn.functional.relu(x) ** 2,
            "sreglu": lambda x: torch.nn.functional.relu(x.chunk(2, -1)[0]) ** 2
            * x.chunk(2, -1)[1],
            "silu": torch.nn.functional.silu,
            "swiglu": lambda x: torch.nn.functional.silu(x.chunk(2, -1)[0])
            * x.chunk(2, -1)[1],
        ...
        if self.activation not in activation_map:
            raise ValueError(
                f"Unsupported activation in onnx export: {self.activation}"
            )
        ...
        def make_debug(prefix, offset):
            ...
            return [
                DebugQuantizer(
                    ...
                    label,
                    None
                    if label in ("dgrad", "wgrad")
                    else base_quantizers[i + offset],
                    self.tp_group,
                )
    ...
    def _customize_quantizers_float8_current_scaling(
        ...
        assert recipe.float8_current_scaling(), (
    ...
    def _customize_quantizers_float8_blockwise_scaling(
        self, fwd: bool, recipe: Recipe
    ) -> None:
        """Customize quantizers based on blockwise scaling recipe + layernorm_mlp."""
        assert recipe.float8_block_scaling(), (
